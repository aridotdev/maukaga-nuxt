...
const actionButtonClass = 'flex max-w-lg items-center justify-center gap-2 rounded-full border border-transparent bg-[#0F172A] px-10 py-6 text-lg font-semibold text-white shadow-sm transition-all duration-400 ease-out group-hover:scale-[1.02] group-hover:border-white/80 group-hover:bg-white/65 group-hover:text-[#0F172A] group-hover:shadow-[0_24px_48px_rgba(15,23,42,0.08)] group-hover:backdrop-blur-2xl md:w-auto'
</script>

<template>
  <!-- LAUNCHER CENTRAL PORTAL -->
  <section class="mx-auto flex w-full flex-col items-center justify-center mt-10 py-10 md:py-0 gap-20">
    <!-- Title & Greeting Section -->
    <div class="mb-8 max-w-2xl px-4 text-center md:mb-12">
      <h2 class="my-3 text-3xl font-bold tracking-tight text-[#0F172A] md:mb-4 md:text-5xl leading-16">
    ...
    <div class="flex flex-col max-w-4xl gap-6 items-center">
      <!-- CTA BUAT PERMINTAAN BARU -->
      <NuxtLink to="/new" class="group">
        <span :class="actionButtonClass">
          <span class="text-2xl">Buat Permintaan Baru</span>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-8 w-8 transition-transform duration-400 ease-out group-hover:scale-[1.02]"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="2"
      ...
      <p class="text-sm text-[#64748B] md:text-base">
        Atau 
        <NuxtLink to="/check-status" class="font-semibold text-[#0F172A] underline hover:no-underline">
          Cek Status Pengajuan
        </NuxtLink>
        kartu garansi yang telah dibuat sebelumnya.
      </p>
    </div>
  </section>
</template>
